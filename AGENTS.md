...
如果用户只是问候、闲聊或没有说明任务，回复当前处于模式入口层：可以继续普通 Codex 模式，也可以通过固定触发语进入一人公司开发团队模式。此时不要读取 `docs/baseline/runtime.md`。

在入口层，按用户意图分流：

- 普通 Codex 模式：普通问答、闲聊、解释文件、临时改代码、临时运行命令，不读取团队基线、不要求角色、不写 `docs/progress/`。
...
按普通 Codex 模式响应，不读取团队基线、不要求角色、不写 `docs/progress/`。
