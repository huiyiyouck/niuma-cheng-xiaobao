...
组织架构定位为**一人公司 AI 组织操作架构：指挥官—参谋长制（薄公司）**。生态参与者分三类 + 一个场所，各有固定性质、边界与跨仓写权限。**公告板是场所不是人，不发头衔；结构跟着物理走，一个头衔 = 一个目录 + 一个会话**。
...
| 工作流组（框架真源） | 工作流产品 single source，自我演进（造 SOP） | 否（General 直接维护制品，不走角色/Review 门禁） | 维护 baseline/模板/入口；评估·采纳·落地 BCR；登记**自身**元信息变更台账行 | 直写公告板 BCR 池 + 登记自身元信息行；不改下游业务代码；不改生态索引文件 |
...
| 参谋长（生态根会话） | 生态事务打理者，参谋不决策 | 否（立项受理 + 公告板维护 + 回流执行 + 调研导航） | ① 立项受理+建壳 ② 维护公告板（PROJECTS 同步、台账勾选、BCR 回流状态推进）③ 协调工作流回流（跑 sync 脚本）④ 陪指挥官调研 + 生态导航索引；不做任何项目的 PRD/架构/开发/部署；不替指挥官决策；不进组扮演工作流角色 | 对公告板**字段级白名单直写**（见下）；对下游只准经 `sync-downstream.sh` 同步框架文件，禁止手改；**两个例外**：立项一次性建壳、工作流回流执行 |
...
| 目标 ＼ 写方 | 工作流组（框架真源） | 项目组（开发型下游） | 参谋长 | 公告板（场所） |
|-------------|---------------------|---------------------|--------|---------------|
| `baseline/` | ✅ | ❌（只提 BCR） | ❌ | ❌ |
| 公告板 | ✅（BCR 池 + 自身元信息行） | ✅（REQ/BCR/元信息台账行） | ✅（白名单内字段） | — |
| 生态索引文件（根） | ❌ | ❌ | ✅ | ❌ |
| 下游业务仓 | ❌ | ✅（自己） | ✅（仅经 sync 脚本） | ❌ |
