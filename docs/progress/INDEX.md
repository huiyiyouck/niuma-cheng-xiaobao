...
| 优先级 | 待办 | 归属角色 | 来源 | 状态 |
|--------|------|----------|------|------|
| P2 | 数据库迁移机制规范化：建立 drizzle 迁移文件管理 + 部署自动迁移步骤（v0.4 #B1 教训）+ 评估 #B2 替代方案（两步查询拆 FOR UPDATE） | DevOps（主）+ Architect（评估） | [v0.4 测试报告](iterations/v0.4-test-report.md) §「正式修复建议」 / PM 2026-05-31 决定 | ✅ **Step 1 已拍板**（Architect Review 2026-05-31 通过，详见 [DevOps 提案](ad-hoc/2026-05-31-devops-proposal-db-migration-mechanism.md) §「Architect Review 记录区」+ 首条 [ADR-001](../baseline/architecture.md)）。<br/>✅ **#B2 评估完成**（Architect 2026-05-31）：保留当前实现 `FOR UPDATE OF cs`，不采纳两步查询替代方案。理由：当前实现已通过三道 Review + 测试，方案 Y 无实质优势（多 1 次往返、可移植性论点在 ADR-001 锁定 PG 后失效）。建议 Developer 顺手把 `sources.ts:161` 注释加强即可，非必须。<br/>✅ **Step 2 已完成**（Developer 2026-05-31）：`server/drizzle/0000_baseline.sql` 已产出；npm scripts `db:generate`/`db:migrate` 已加；老 SQL 5 个 git mv 到 `_legacy/`，`server/db/migrations/v0.4.sql` 已删；详见 [DevOps 提案 §Step 2 — Developer 实施完成](ad-hoc/2026-05-31-devops-proposal-db-migration-mechanism.md) 和 [developer.md 2026-05-31 Step 2](roles/developer.md)；**有 1 项 Architect 决策微调待确认**（baseline 直接吸收 v0.4 而非独立 0001 增量）<br/>剩余动作：⏳ **Step 3 → DevOps**（A2 移依赖 + systemd `ExecStartPre` + `StartLimitInterval/Burst` + 首次部署 baseline 注入 `__drizzle_migrations` + 操作手册 `docs/knowledge/devops/db-migration-handbook.md`）。<br/>📋 **Step 3 实施计划初稿已写入** [`devops.md` 2026-05-31 P2 评估段末尾](roles/devops.md) + Developer 已在 [DevOps 提案 §Step 2 末尾](ad-hoc/2026-05-31-devops-proposal-db-migration-mechanism.md) 显式提醒硬前置风险 |
