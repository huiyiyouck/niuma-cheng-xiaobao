...
| 日期 | 角色 | 工作 | 结论 | 下一步入口 |
|------|------|------|------|------------|
| 2026-05-31 | WM | 三项基线修正提案合并落地（#1 受保护路径删除 Review 门禁 / #2 前后端契约变更同步检查 / #3 INDEX 跨任务待办模板化） | commit dfede2b 推送 origin/main；11 文件净增 136 行；触发源 2026-05-30 基线同步事故 + v0.3 砍 WS 后前端残留 5 个月 + Developer 元流程提案 | 新约束即刻生效；v0.4 测试阶段继续 |
...
| 优先级 | 待办 | 归属角色 | 来源 | 状态 |
|--------|------|----------|------|------|
| P2 | 数据库迁移机制规范化：建立 drizzle 迁移文件管理 + 部署自动迁移步骤（v0.4 #B1 教训）+ 评估 #B2 替代方案（两步查询拆 FOR UPDATE） | DevOps（主）+ Architect（评估） | [v0.4 测试报告](iterations/v0.4-test-report.md) §「正式修复建议」 / PM 2026-05-31 决定 | ✅ **Step 1 已拍板**（Architect Review 2026-05-31 通过，详见 [DevOps 提案](ad-hoc/2026-05-31-devops-proposal-db-migration-mechanism.md) §「Architect Review 记录区」+ 首条 [ADR-001](../baseline/architecture.md)）。<br/>✅ **#B2 评估完成**（Architect 2026-05-31）：保留当前实现 `FOR UPDATE OF cs`，不采纳两步查询替代方案。理由：当前实现已通过三道 Review + 测试，方案 Y 无实质优势（多 1 次往返、可移植性论点在 ADR-001 锁定 PG 后失效）。建议 Developer 顺手把 `sources.ts:161` 注释加强即可，非必须。<br/>剩余动作：⏳ **Step 2 → Developer**（baseline introspect + 归并 v0.4.sql 为 0001 + 老路径清理；含 A1 加 db:generate/db:migrate scripts + A3 generate 后比对 v0.4.sql）→ ⏳ **Step 3 → DevOps**（A2 移依赖 + systemd `ExecStartPre` + `StartLimitInterval/Burst` + 操作手册 `docs/knowledge/devops/db-migration-handbook.md`）。<br/>📋 **Step 3 实施计划初稿已写入** [`devops.md` 2026-05-31 P2 评估段末尾](roles/devops.md)，等 Step 2 完成即可开工 |
